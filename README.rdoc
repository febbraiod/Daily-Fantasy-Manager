...
DO THIS ---- for roto lineup builder: make it so we can take in the csv template from roto and adjust their projections automagically, throw out players not in our db and output the result.  For GPP if we dont have a ceiling proj, we keep the roto projection. If we dont have a floor project we make the roto projection 0.

in this vein how about a method to comibined the multiple csv's from roto and FFP
